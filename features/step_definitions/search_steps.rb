...
Then(/^the result should be adjusted according to TV size$/) do
...
When(/^I select a minimum TV price of "([^"]*)"$/) do |price|
  @tv_page.select(price, :from => "search[range][price][price][lower]" )
